...
const solution = (input) => {
  ...
  const availableSize = 70_000_000;
  const requiredSize = 30_000_000;
  const totalSize = sizes[0].size
  const toBeDeleted = totalSize - availableSize + requiredSize

  return sizes.filter(({ size }) => toBeDeleted < size).sort((a, b) => a.size - b.size)[0].size
};
